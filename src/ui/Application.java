...
public class Application extends JFrame {
    ...
    public void addQuery(Query query) {
        queries.add(query); // save the query
        Set<String> allterms = getQueryTerms(); // filter all terms for twitter api
        twitterSource.setFilterTerms(allterms); // give the terms to twitter api
        ...
        map().addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                if (markers.size() > 0) {
                    ...
                    map().setToolTipText(getToolTipContent((MapMarkerSimple) m));
                } else {
                    map().setToolTipText("hover over tweet markers to read a tweet");
                }
            }
        });
    }

    private String getToolTipContent(MapMarkerSimple simpleMarker) {
        Status s = simpleMarker.getTweet();
        String imageUrl = s.getUser().getMiniProfileImageURL();
        String htmlStr = "<html><img src="+imageUrl+" /><span>"+s.getText()+"</span></html>";
        return htmlStr;
    }
